...
public:
    ...
    void GetGridDimensions(int dimesions[3]) const {memcpy(dimesions, grid_dimensions_, 3 * sizeof(int));}
    ...
    bool IsInMap(const double ixyz[3]) const;
...
private:
    ...
    void ShiftOrigin(const int new_lower_lesft_voxels[3]);
    ...
    void InitializeData()
    {
        ...
        for(int i = 0 ; i < num_cells_; i++) data_.push_back(T());
    }
...
void VoxelGrid<T>::ComputeLowerLeftVoxels(const double new_grid_center_xyz[3], int new_lower_left_voxels[3]) const
{
    ...
    WorldToGrid(new_grid_center_xyz, new_grid_center_voxels);
    for( int i =0 ; i <3; i++)
    {
        new_lower_left_voxels[i] = new_grid_center_voxels[i] - (grid_dimensions_[i] >> i );
    }
}
...
int VoxelGrid<T>::GridToIndex(const int ixyz[3]) const
{
    ...
    for ( int i =0; i< 3; i++)
    {
        ixyz_offset[i] = (ixyz[i] + lower_left_voxels_[i]);
        if(ixyz_offset[i] < 0)
        {
            ixyz_offset[i] += grid_dimension_[i];
        }
    }
    int index = ixyz_offset[2]*(grid_dimensions_[0]*grid_dimensions_[1])
              + ixyz_offset[1]*(grid_dimensions_[0])
              + ixyz_offset[0];
    return index;
}
